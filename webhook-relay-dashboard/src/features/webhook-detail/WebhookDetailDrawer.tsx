...
export function WebhookDetailDrawer() {
  ...
  // Fetch event details only when the drawer is open and an ID is selected
  ...
  // Imperative control of the native <dialog> element
  useEffect(() => {
    const dialog = dialogRef.current;
    if (!dialog) return;

    if (isDetailDrawerOpen && !dialog.open) {
      dialog.showModal(); // Native focus trapping & backdrop
    } else if (!isDetailDrawerOpen && dialog.open) {
      dialog.close();
    }
  }, [isDetailDrawerOpen]);

  // Elite Directive: Explicit cleanup on unmount to prevent detached DOM nodes
  useEffect(() => {
    return () => {
  ...
  return (
    // Native dialog handles backdrop and focus trap. 
    // CSS handles the slide-in animation (GPU composited).
    <dialog 
      ref={dialogRef} 
      onClose={closeDetailDrawer} // Fires when user presses ESC or clicks backdrop
        ...
        <main className="flex-1 overflow-y-auto px-6 py-6 space-y-6 bg-canvas">
          {isLoading ? (
            ...
            <>
              ...
              {/* Event Metadata - Apple Store Utility Card Style */}
              ...
              {/* Payload Viewer (Offloaded to Web Worker) */}
              <section>
