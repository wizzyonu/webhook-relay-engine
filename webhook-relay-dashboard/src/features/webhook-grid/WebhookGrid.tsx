...
export function WebhookGrid() {
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, isError } = useWebhooks();
  ...
  return (
    // Directive: Zero CLS. The parent container must have a strict, defined height.
    // We use the Apple 'canvas' background for the data area.
    <ErrorBoundary fallback={<div className="p-8 text-red-500">Grid Crashed</div>}>
      <div 
        ref={parentRef} 
        className="h-[calc(100vh-52px)] overflow-auto contain-strict bg-canvas"
      >
        <div
          style={{
